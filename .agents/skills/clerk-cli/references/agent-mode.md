...
| All other mutation confirmations (`config patch` / `put`, `api -X POST/PATCH/DELETE`, `users create`, `enable` / `disable`) | Prompt y/n | **Live mutations require explicit user confirmation before the agent executes them.** `--dry-run` is the required preview step; `--yes` is not a safety gate and does not replace the confirmation flow.                                                                                                                                                                                                                                                                                                                             |
...
**Rule of thumb:** preview every mutation with `--dry-run`, then only proceed after explicit user confirmation. Use `--json` for structured output where available, and pass `--app` / `--instance` when you intentionally target a real app; pass `--keyless` to opt into auto-generated dev keys when bootstrapping a new project without authenticating.
